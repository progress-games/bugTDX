local elite = Object:extend()

function elite:new(path, x, y, name)
    self.path = path
    ...
    self.name = name
    self.scale = 5
    if config.difficulty == 4 and name == 'beetle' then
        self.hp = self.hp * 2.5
...
function elite:update(dt)
    ...
    for e, s in pairs(self.effects) do
        if e == 'burn' or e == 'lava' or e == 'flamingoil' then
            if s.ratetimer == nil then s.ratetimer = 2*dt; s.ratedur = 0 end
            ...
            if s.ratedur <= 0 then self.effects[e] = nil end
        end
        if e == 'oil' then
            c = c - s.slow
        ...
        if e == 'confused' then
            s.rate = s.rate - dt
            if s.rate <= 0 then self.effects['confused'] = nil end
        end
        if e == 'stone' then
            c = c - s.slow
...
function elite:takedmg(dmg, effect, stats)
    ...
    if effect == 'shock' and self.effects['water'] then 
...
function elite:draw(x, y)
    ...
    if self.unit == 'air' then
        ...
        love.graphics.draw(self.sprite, self.x, self.y, self.dir+math.pi/2, 0.5*self.scale, 0.5*self.scale, 16, 16)
        love.graphics.setColor(1, 1, 1)
        if self.flash.status then
            love.graphics.setShader(hitflash)
            love.graphics.draw(self.sprite, self.x, self.y-8, self.dir+math.pi/2, 0.5*self.scale, 0.5*self.scale, 16, 16)
    ...
    if #self.effects ~= 0 then
        love.graphics.setColor(1, 1, 1)
        love.graphics.rectangle('fill', self.x-35, self.y-21, 17*self.scale, 5, 3, 3)
        love.graphics.setColor(0, 0, 0)
        love.graphics.rectangle('fill', self.x-33, self.y-20, 17*self.scale-4, 3, 3, 3)
        love.graphics.setColor(1, 0, 0)
        love.graphics.rectangle('fill', self.x-33, self.y-20, self.hp/(self.maxhp)*(17*self.scale)-4, 3, 3, 3)
        love.graphics.setColor(1, 1, 1)
    else
        love.graphics.setColor(1, 1, 1)
        love.graphics.rectangle('fill', self.x-35, self.y-13, 17*self.scale, 5, 3, 3)
        love.graphics.setColor(0, 0, 0)
        love.graphics.rectangle('fill', self.x-33, self.y-12, 17*self.scale-4, 3, 3, 3)
        love.graphics.setColor(1, 0, 0)
        love.graphics.rectangle('fill', self.x-33, self.y-12, self.hp/(self.maxhp)*(17*self.scale)-4, 3, 3, 3)
        love.graphics.setColor(1, 1, 1)
    end
    local i = 1
    for e, s in pairs(self.effects) do
        if e == 'wet' then love.graphics.draw(sprites.water, self.x-32+i*7, self.y - 27)
        elseif e == 'burn' or e == 'lava' then love.graphics.draw(sprites.flame, self.x-32+i*7, self.y - 27)
        elseif e == 'oil' then love.graphics.draw(sprites.oil, self.x-32+i*7, self.y - 27)
        elseif e == 'freeze' then love.graphics.draw(sprites.freeze, self.x-32+i*7, self.y - 27)
        elseif e == 'slow' then love.graphics.draw(sprites.slow, self.x-32+i*7, self.y - 27)
        elseif e == 'push' then love.graphics.draw(sprites.push, self.x-32+i*7, self.y - 27)
        elseif e == 'flamingoil' then love.graphics.draw(sprites.flamingoil, self.x-32+i*7, self.y - 27)
        elseif e == 'weaken' then love.graphics.draw(sprites.fistbullet, self.x-32+i*7, self.y - 27, 0, 0.5, 0.5)
        elseif e == 'poison' then love.graphics.draw(sprites.poisondrop, self.x-32+i*7, self.y - 27, 0, 0.5, 0.5)
        elseif e == 'confusion' then love.graphics.draw(sprites.confusion, self.x-32+i*7, self.y - 27, 0, 0.5, 0.5)
        elseif e == 'stone' then love.graphics.draw(sprites.stone, self.x-32+i*7, self.y - 27, 0, 0.5, 0.5)
        elseif e == 'electrocution' then love.graphics.draw(sprites.electrocution, self.x-32+i*7, self.y - 27, 0, 0.5, 0.5)
        elseif e == 'detonator' then love.graphics.draw(sprites.dynamite, self.x-32+i*7, self.y - 27, 0, 0.5, 0.5) end
